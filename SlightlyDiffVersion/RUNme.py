...
class ButtonThing:
    ...
    def __init__(self,master,app):
        ...
        self.label = gwaphics.Label(master, text="Model")
        self.label.pack()

        self.findCritical = gwaphics.Button(master,text="Find Critical",command=self.app.changeColor)
        self.findCritical.pack(side = "top") # would show users people that lies on the critical point
        ...
        self.startInfection_button.pack(side = "top") # this would call a method to call the infection in main

        self.startInoculate_button = gwaphics.Button(master, text="Start Inoculate",
                                                     command=app.inoculatePeople)  # this can call a method which starts inoculates
        self.startInoculate_button.pack(side="top")

        # self.startover_button = gwaphics.Button(master,text="Startover", command=app.)
...
class coordStuff:
    """this class helps to find out the coord the virus needs to move for each step"""
    ...
    def moveOval(self): # this help to call move oval
        if self.count < 10:
            self.oval.moveTo()
...
class Application:
    def __init__(self,master):
        self.master = master
        self.criticalColor = "red" # some attributes
    ...
    def connectPeople(self, population):
        """loop over people list to connect people"""
    ...
    def moveVirus(self,A,B):
        """move the coord of virus"""
        for k, v in A.items(): # unpack the information of the dictionary get from main
    ...
    def subInoculate(self):
        """a helper"""
        self.inoculateA, self.inoculateB = simulate.inoculation()
